...
class TransactionGatewayAbstract:
    ...
    def get_short_name(self):
        '''Must be unique, we don't have a registry for this, obviously, 
        but it makes sense to provide Organisationally Unique Identifier (OUI)'s
        for this to allow systems to distinguish uniqe records accross systems
        whilst still retaining their origin (different from using a uuid, which
        would provide uniqueness, but not reference).'''
        raise NotImplementedError()
